<?php
/**
 * includes/migrations/class-wp-mcp-ai-migrate-medical-record-post-type.php (ecosystem port — Wave F4, healthcare toolkit data layer).
 *
 * Ported from the base Pro addon's `addons/pro/includes/migrations/class-wp-mcp-ai-migrate-medical-record-post-type.php` for the standalone `nvoos-content-graph-pro`
...
class WP_MCP_AI_Migrate_Medical_Record_Post_Type {
	...
	public static function run() {
		...
		if ( 0 === $total_posts ) {
			...
			return array(
				'status'   => 'success',
				'message'  => __( 'No medical records found to migrate.', 'nvoos-content-graph-pro' ),
				'migrated' => 0,
				'total'    => 0,
			);
		}
		...
		return array(
			...
			'message'  => sprintf(
				...
				__( 'Successfully migrated %d medical records.', 'nvoos-content-graph-pro' ),
				$updated
			),
			'migrated' => $updated,
			'total'    => $total_posts,
	...
	public static function rollback() {
		...
		if ( 0 === $total_posts ) {
			...
			return array(
				'status'      => 'success',
				'message'     => __( 'No medical records found to rollback.', 'nvoos-content-graph-pro' ),
				'rolled_back' => 0,
				'total'       => 0,
			);
		}
		...
		return array(
			...
			'message'     => sprintf(
				...
				__( 'Successfully rolled back %d medical records.', 'nvoos-content-graph-pro' ),
				$updated
			),
			'rolled_back' => $updated,
			'total'       => $total_posts,
