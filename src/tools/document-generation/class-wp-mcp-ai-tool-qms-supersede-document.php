<?php
/**
 * WP_MCP_AI_Tool_QMS_Supersede_Document (ecosystem port - Wave F2, document-generation tool batch).
 *
...
class WP_MCP_AI_Tool_QMS_Supersede_Document implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'QMS: Supersede Document', 'nvoos-content-graph-pro' );
	}
	/**
	...
	public function get_description() {
		return __( 'Mark a released controlled document as superseded by a new revision. The new record (successor_post_id) must already exist; the previous record is left in `superseded` state and linked.', 'nvoos-content-graph-pro' );
	}
		/**
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $post_id || ! $successor || $post_id === $successor ) {
			return new WP_Error( 'wp_mcp_ai_invalid', __( 'Valid post_id and successor_post_id are required and must differ.', 'nvoos-content-graph-pro' ) );
		}
		$succ = get_post( $successor );
		if ( ! $succ || WP_MCP_AI_QMS_Doc_Record_CPT::POST_TYPE !== $succ->post_type ) {
			return new WP_Error( 'wp_mcp_ai_qms_invalid_successor', __( 'Successor must be a controlled document record.', 'nvoos-content-graph-pro' ) );
		}
