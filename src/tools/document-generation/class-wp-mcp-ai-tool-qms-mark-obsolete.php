<?php
/**
 * WP_MCP_AI_Tool_QMS_Mark_Obsolete (ecosystem port - Wave F2, document-generation tool batch).
 *
...
class WP_MCP_AI_Tool_QMS_Mark_Obsolete implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'QMS: Mark Obsolete', 'nvoos-content-graph-pro' );
	}
	/**
	...
	public function get_description() {
		return __( 'Mark a controlled document as obsolete. The record is preserved (never destroyed) and moved to PARA archives if PARA is enabled.', 'nvoos-content-graph-pro' );
	}
		/**
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! $post_id || '' === $reason ) {
			return new WP_Error( 'wp_mcp_ai_invalid', __( 'post_id and reason are required.', 'nvoos-content-graph-pro' ) );
		}
