<?php
/**
 * WP_MCP_AI_Tool_QMS_Approve_Document (ecosystem port - Wave F2, document-generation tool batch).
 *
...
class WP_MCP_AI_Tool_QMS_Approve_Document implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'QMS: Approve Document', 'nvoos-content-graph-pro' );
	}
	/**
	...
	public function get_description() {
		return __( 'Mark a controlled document as approved. Document must be in in_review state and have at least one approver assigned.', 'nvoos-content-graph-pro' );
	}
		/**
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( ! user_can( $user_id, 'manage_options' ) && ! in_array( $user_id, array_map( 'intval', $approvers ), true ) ) {
			return new WP_Error( 'wp_mcp_ai_qms_not_approver', __( 'Only assigned approvers may approve this document.', 'nvoos-content-graph-pro' ) );
		}
