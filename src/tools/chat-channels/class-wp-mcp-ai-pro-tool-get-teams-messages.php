<?php
/**
 * tools/chat-channels/class-wp-mcp-ai-pro-tool-get-teams-messages.php (ecosystem port — Wave F5, chat-channels tool batch).
 *
 * Ported from the base Pro addon's `addons/pro/includes/tools/chat-channels/class-wp-mcp-ai-pro-tool-get-teams-messages.php` for the
...
class WP_MCP_AI_Pro_Tool_Get_Teams_Messages implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Get Teams Messages', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Retrieves message history from a Microsoft Teams channel using the Microsoft Graph API.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'team_id'    => array(
					'type'        => 'string',
					'description' => __( 'Microsoft Teams team ID to retrieve messages from.', 'nvoos-content-graph-pro' ),
				),
				'channel_id' => array(
					'type'        => 'string',
					'description' => __( 'Microsoft Teams channel ID to retrieve messages from.', 'nvoos-content-graph-pro' ),
				),
				'top'        => array(
					'type'        => 'integer',
					'description' => __( 'Number of messages to retrieve (1-50, default: 50).', 'nvoos-content-graph-pro' ),
					'minimum'     => 1,
					'maximum'     => 50,
					'default'     => 50,
				),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( $required_capability && ( ! $user_id || ! user_can( $user_id, $required_capability ) ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to retrieve Teams messages.', 'nvoos-content-graph-pro' ) );
		}
