<?php
/**
 * tools/chat-channels/class-wp-mcp-ai-pro-tool-remove-google-chat-space-member.php (ecosystem port — Wave F5, chat-channels tool batch).
 *
 * Ported from the base Pro addon's `addons/pro/includes/tools/chat-channels/class-wp-mcp-ai-pro-tool-remove-google-chat-space-member.php` for the
...
class WP_MCP_AI_Pro_Tool_Remove_Google_Chat_Space_Member implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Remove Google Chat Space Member', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Removes a member from a Google Chat space using the Google Chat API v1.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'membership'          => array(
					'type'        => 'string',
					'description' => __( 'Membership resource name to remove (e.g., spaces/SPACE_ID/members/MEMBER_ID).', 'nvoos-content-graph-pro' ),
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( $required_capability && ( ! $user_id || ! user_can( $user_id, $required_capability ) ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to remove Google Chat space members.', 'nvoos-content-graph-pro' ) );
		}
		...
		if ( '' === $membership ) {
			return new WP_Error( 'wp_mcp_ai_missing_membership', __( 'A membership resource name is required.', 'nvoos-content-graph-pro' ) );
		}

		// Validate membership resource name format.
		if ( ! preg_match( '/^spaces\/[a-zA-Z0-9_-]+\/members\/[a-zA-Z0-9_-]+$/', $membership ) ) {
			return new WP_Error( 'wp_mcp_ai_invalid_membership', __( 'Invalid membership format. Expected format: spaces/SPACE_ID/members/MEMBER_ID', 'nvoos-content-graph-pro' ) );
		}
