<?php
/**
 * tools/chat-channels/class-wp-mcp-ai-pro-tool-get-google-chat-spaces.php (ecosystem port — Wave F5, chat-channels tool batch).
 *
 * Ported from the base Pro addon's `addons/pro/includes/tools/chat-channels/class-wp-mcp-ai-pro-tool-get-google-chat-spaces.php` for the
...
class WP_MCP_AI_Pro_Tool_Get_Google_Chat_Spaces implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Get Google Chat Spaces', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Retrieves a list of Google Chat spaces using the Google Chat API v1.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'filter'              => array(
					'type'        => 'string',
					'description' => __( 'Optional filter for spaces (e.g., spaceType = "SPACE").', 'nvoos-content-graph-pro' ),
				),
				'page_size'           => array(
					'type'        => 'integer',
					'description' => __( 'Maximum number of spaces to return per page (1–1000, default 100).', 'nvoos-content-graph-pro' ),
					'default'     => 100,
					'minimum'     => 1,
					'maximum'     => 1000,
				),
				'page_token'          => array(
					'type'        => 'string',
					'description' => __( 'Page token from a previous response to retrieve the next page of spaces.', 'nvoos-content-graph-pro' ),
				),
			),
			'required'             => array(),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( $required_capability && ( ! $user_id || ! user_can( $user_id, $required_capability ) ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to retrieve Google Chat spaces.', 'nvoos-content-graph-pro' ) );
		}
