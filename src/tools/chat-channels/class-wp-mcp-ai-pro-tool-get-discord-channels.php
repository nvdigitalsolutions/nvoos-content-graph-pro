<?php
/**
 * tools/chat-channels/class-wp-mcp-ai-pro-tool-get-discord-channels.php (ecosystem port — Wave F5, chat-channels tool batch).
 *
 * Ported from the base Pro addon's `addons/pro/includes/tools/chat-channels/class-wp-mcp-ai-pro-tool-get-discord-channels.php` for the
...
class WP_MCP_AI_Pro_Tool_Get_Discord_Channels implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Get Discord Channels', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Retrieves a list of channels from a Discord server (guild) using the Discord Bot API.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'guild_id' => array(
					'type'        => 'string',
					'description' => __( 'Discord server (guild) ID to retrieve channels from.', 'nvoos-content-graph-pro' ),
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( $required_capability && ( ! $user_id || ! user_can( $user_id, $required_capability ) ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to retrieve Discord channels.', 'nvoos-content-graph-pro' ) );
		}
