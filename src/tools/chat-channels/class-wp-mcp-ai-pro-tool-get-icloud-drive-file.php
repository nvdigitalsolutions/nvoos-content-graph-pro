<?php
/**
 * tools/chat-channels/class-wp-mcp-ai-pro-tool-get-icloud-drive-file.php (ecosystem port — Wave F5, chat-channels tool batch).
 *
 * Ported from the base Pro addon's `addons/pro/includes/tools/chat-channels/class-wp-mcp-ai-pro-tool-get-icloud-drive-file.php` for the
...
class WP_MCP_AI_Pro_Tool_Get_Icloud_Drive_File implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Get iCloud Drive File', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Retrieves metadata and download information for a file from iCloud Drive via a configured iCloud gateway API.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'file_id'     => array(
					'type'        => 'string',
					'description' => __( 'File identifier in iCloud Drive.', 'nvoos-content-graph-pro' ),
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( $required_capability && ( ! $user_id || ! user_can( $user_id, $required_capability ) ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to retrieve iCloud Drive files.', 'nvoos-content-graph-pro' ) );
		}
		...
		if ( '' === $file_id ) {
			return new WP_Error( 'wp_mcp_ai_missing_icloud_file_id', __( 'A file ID is required.', 'nvoos-content-graph-pro' ) );
		}
