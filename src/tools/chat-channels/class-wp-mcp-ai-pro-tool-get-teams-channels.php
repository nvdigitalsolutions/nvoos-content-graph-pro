<?php
/**
 * tools/chat-channels/class-wp-mcp-ai-pro-tool-get-teams-channels.php (ecosystem port — Wave F5, chat-channels tool batch).
 *
 * Ported from the base Pro addon's `addons/pro/includes/tools/chat-channels/class-wp-mcp-ai-pro-tool-get-teams-channels.php` for the
...
class WP_MCP_AI_Pro_Tool_Get_Teams_Channels implements WP_MCP_AI_Tool_Interface, WP_MCP_AI_Tool_Capability_Flags_Interface {
	...
	public function get_name() {
		return __( 'Get Teams Channels', 'nvoos-content-graph-pro' );
	}
	...
	public function get_description() {
		return __( 'Retrieves a list of channels from a Microsoft Teams team using the Microsoft Graph API.', 'nvoos-content-graph-pro' );
	}
	...
	public function get_parameters_schema() {
		return array(
			...
			'properties'           => array(
				...
				'team_id' => array(
					'type'        => 'string',
					'description' => __( 'Microsoft Teams team ID to retrieve channels from.', 'nvoos-content-graph-pro' ),
				),
			),
	...
	public function execute( array $arguments = array(), array $context = array() ) {
		...
		if ( $required_capability && ( ! $user_id || ! user_can( $user_id, $required_capability ) ) ) {
			return new WP_Error( 'wp_mcp_ai_forbidden', __( 'You do not have permission to retrieve Teams channels.', 'nvoos-content-graph-pro' ) );
		}
