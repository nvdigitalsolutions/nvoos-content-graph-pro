...
if ( ! class_exists( 'WP_MCP_AI_Tool_Cloudways_Server_Monitor_Summary' ) ) {
	...
	class WP_MCP_AI_Tool_Cloudways_Server_Monitor_Summary extends WP_MCP_AI_Tool_Cloudways_Base {
		...
		public function get_name() {
			return __( 'Server Monitoring Summary', 'nvoos-content-graph-pro' );
		}

		/** {@inheritdoc} */
		public function get_description() {
			return __( 'View server bandwidth and disk usage metrics.', 'nvoos-content-graph-pro' );
		}
		...
		public function execute( array $arguments = array(), array $context = array() ) {
			...
			return $this->success(
				sprintf(
					...
					__( 'Monitoring summary retrieved for server %d.', 'nvoos-content-graph-pro' ),
					$server_id
				),
				array( 'summary' => $data )
			);
