...
if ( ! class_exists( 'WP_MCP_AI_Tool_Cloudways_Git_Pull' ) ) {
	...
	class WP_MCP_AI_Tool_Cloudways_Git_Pull extends WP_MCP_AI_Tool_Cloudways_Base {
		...
		public function get_name() {
			return __( 'Git Pull', 'nvoos-content-graph-pro' );
		}

		/** {@inheritdoc} */
		public function get_description() {
			return __( 'Pull the latest commits from the linked Git branch and deploy them onto an application.', 'nvoos-content-graph-pro' );
		}
		...
		public function execute( array $arguments = array(), array $context = array() ) {
			...
			return $this->success(
				sprintf(
					...
					__( 'Git pull initiated for app %d.', 'nvoos-content-graph-pro' ),
					$app_id
				),
				$result
			);
