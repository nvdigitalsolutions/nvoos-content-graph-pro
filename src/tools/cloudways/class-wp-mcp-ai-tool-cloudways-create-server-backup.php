...
if ( ! class_exists( 'WP_MCP_AI_Tool_Cloudways_Create_Server_Backup' ) ) {
	...
	class WP_MCP_AI_Tool_Cloudways_Create_Server_Backup extends WP_MCP_AI_Tool_Cloudways_Base {
		...
		public function get_name() {
			return __( 'Create Server Backup', 'nvoos-content-graph-pro' );
		}

		/** {@inheritdoc} */
		public function get_description() {
			return __( 'Create a full backup of a server.', 'nvoos-content-graph-pro' );
		}
		...
		public function execute( array $arguments = array(), array $context = array() ) {
			...
			return $this->success(
				sprintf(
					...
					__( 'Backup initiated for server %d.', 'nvoos-content-graph-pro' ),
					$server_id
				),
				$result
			);
