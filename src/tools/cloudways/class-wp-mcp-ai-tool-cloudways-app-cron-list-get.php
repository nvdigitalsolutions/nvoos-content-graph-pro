...
if ( ! class_exists( 'WP_MCP_AI_Tool_Cloudways_App_Cron_List_Get' ) ) {
	...
	class WP_MCP_AI_Tool_Cloudways_App_Cron_List_Get extends WP_MCP_AI_Tool_Cloudways_Base {
		...
		public function get_name() {
			return __( 'Get App Cron List', 'nvoos-content-graph-pro' );
		}

		/** {@inheritdoc} */
		public function get_description() {
			return __( 'Retrieve the list of cron jobs for an application.', 'nvoos-content-graph-pro' );
		}
		...
		public function execute( array $arguments = array(), array $context = array() ) {
			...
			return $this->success(
				sprintf(
					...
					_n( 'Found %d cron job.', 'Found %d cron jobs.', count( $cron_jobs ), 'nvoos-content-graph-pro' ),
					count( $cron_jobs )
				),
