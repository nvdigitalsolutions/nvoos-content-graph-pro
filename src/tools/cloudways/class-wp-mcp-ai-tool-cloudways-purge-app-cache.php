...
if ( ! class_exists( 'WP_MCP_AI_Tool_Cloudways_Purge_App_Cache' ) ) {
	...
	class WP_MCP_AI_Tool_Cloudways_Purge_App_Cache extends WP_MCP_AI_Tool_Cloudways_Base {
		...
		public function get_name() {
			return __( 'Purge App Cache', 'nvoos-content-graph-pro' );
		}

		/** {@inheritdoc} */
		public function get_description() {
			return __( 'Clear all cache layers for an application (Varnish, object cache, etc.).', 'nvoos-content-graph-pro' );
		}
		...
		public function execute( array $arguments = array(), array $context = array() ) {
			...
			return $this->success(
				sprintf(
					...
					__( 'Cache purged for app %d.', 'nvoos-content-graph-pro' ),
					$app_id
				),
				$result
			);
